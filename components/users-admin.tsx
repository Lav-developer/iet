...
export function UsersAdmin() {
  ...
  useEffect(() => { load(1); }, []);
  // Who is signed in decides which actions this screen offers. The API
  // re-authorizes every change, so this only keeps unusable controls out of the
  // way (self-deactivation and super-admin accounts an IET admin cannot touch).
  useEffect(() => { fetch("/api/auth/me").then((response) => response.ok ? response.json() : null).then((body) => setActor(body?.user || null)).catch(() => setActor(null)); }, []);
  useEffect(() => {
  ...
  return <>
    ...
    <div className="admin-panel">
      ...
      <form className="form-grid" onSubmit={create}>
        ...
        <div className="form-field"><label htmlFor="user-role">Role</label><select id="user-role" className="form-select" value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value, departmentId: "" })}>{adminRolesFor(actor).map((role) => <option key={role} value={role}>{role.replaceAll("_", " ")}</option>)}</select><span className="form-hint">{form.role === "DEPARTMENT_ADMIN" ? "Scoped to one department." : "Institute-wide role — no department is assigned."}</span></div>
