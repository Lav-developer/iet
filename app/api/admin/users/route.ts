...
export async function GET() {
  try {
    ...
    const users = await prisma.user.findMany({ orderBy: { name: "asc" }, select: { id: true, email: true, name: true, role: true, departmentId: true, active: true, sessionVersion: true, createdAt: true, updatedAt: true, department: { select: { slug: true, name: true } } } });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!limit.allowed) return NextResponse.json({ error: "Too many user changes. Try again later." }, { status: 429, headers: { "Retry-After": String(limit.retryAfterSeconds) } });
    ...
    const user = await prisma.user.create({ data: { email: input.email.toLowerCase(), name: input.name, passwordHash, role: input.role, departmentId: input.departmentId || null }, select: { id: true, email: true, name: true, role: true, departmentId: true, active: true, sessionVersion: true } });
    await audit(prisma, actor, "CREATED", user.id, { email: user.email, role: user.role });
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const user = await prisma.user.update({ where: { id: target.id }, data: { name: body.data.name, passwordHash, role: body.data.role, departmentId: body.data.departmentId, active: body.data.active, ...(mustInvalidate ? { sessionVersion: { increment: 1 } } : {}) }, select: { id: true, email: true, name: true, role: true, departmentId: true, active: true, sessionVersion: true } });
    await audit(prisma, actor, mustInvalidate ? "UPDATED_AND_INVALIDATED_SESSIONS" : "UPDATED", user.id, { email: user.email, role: user.role, active: user.active });
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const user = await prisma.user.update({ where: { id: target.id }, data: { active: false, sessionVersion: { increment: 1 } }, select: { id: true, email: true, name: true, role: true, active: true } });
    await audit(prisma, actor, "DEACTIVATED_AND_INVALIDATED_SESSIONS", user.id, { email: user.email });
    return NextResponse.json({ user });
  } catch (error) {
...
async function audit(prisma: NonNullable<ReturnType<typeof getPrisma>>, actor: { id: string; role: string }, action: string, entityId: string, after: unknown) {
  await prisma.auditLog.create({ data: { userId: actor.id, role: actor.role as any, action, entity: "users", entityId, afterJson: JSON.stringify(after) } });
}
