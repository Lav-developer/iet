...
export async function GET() {
  ...
  const urls = ["/", "/notices", "/about", "/infrastructure", "/departments", "/programs", "/faculty", "/people", "/search", "/laboratories", "/research", "/projects", "/publications", "/events", "/achievements", "/campus", "/organizations", "/admissions", "/career", "/resources", "/contact", "/accessibility", "/privacy", ...data.organizations.map((item) => `/organizations/${item.slug}`), ...data.pages.filter((item) => item.locale === "en").map((item) => `/${item.slug}`), ...data.notices.map((item) => `/notices/${item.slug}`),
