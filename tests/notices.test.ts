...
test("notices use the existing editorial workflow and department scoping", () => {
  ...
  assert.equal(canAccess({ role: "EDITOR" }, "notices", "write", { status: "PUBLISHED" }, own), true, "editors publish within the editorial scope");
  assert.equal(canAccess({ role: "EDITOR" }, "notices", "write", { status: "DRAFT" }, { ...own, status: "PUBLISHED" }), false, "editors do not unpublish");
  ...
  assert.equal(workflowTransitionAllowed({ role: "EDITOR" }, { ...own, status: "REVIEW" }, "PUBLISHED", { entity: "notices", departmentSlug: own.departmentSlug }), true);
  assert.equal(workflowTransitionAllowed({ role: "EDITOR" }, { ...own, status: "REVIEW" }, "PUBLISHED"), false, "without a record scope no delegated authority is assumed");
  assert.equal(workflowTransitionAllowed({ role: "DEPARTMENT_ADMIN", departmentId: "dept-1" }, { ...own, status: "REVIEW" }, "PUBLISHED", { entity: "notices", departmentSlug: own.departmentSlug, assignedDepartmentSlug: "computer-science-engineering" }), true);
  assert.equal(workflowTransitionAllowed({ role: "DEPARTMENT_ADMIN", departmentId: "dept-1" }, { ...own, status: "REVIEW" }, "PUBLISHED", { entity: "notices", departmentSlug: "civil-engineering", assignedDepartmentSlug: "computer-science-engineering" }), false);
